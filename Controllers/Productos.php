...
class Productos extends Controller{                         #clase usuarios heredados clase controller     
    ...
    public function registrar()
    {
        ...
        if (empty($codigo) || empty($nombre)|| empty($descripcion) || empty($precio_creacion) || 
            empty($precio_venta) || empty($medida)|| empty($categoria)) {
            $msg = "Todos los campos son obligatorios";  
        }else{
            if ($id == "") {
                $data = $this->model->registrarProducto($codigo,$nombre,$descripcion,$precio_creacion,$precio_venta,$medida,$categoria);            //trae el metodo registrar producto de Model
                if ($data == "ok") {
                    $msg = "si";
                }else if($data == "existe"){
                ...
                $data = $this->model->modificarProducto($codigo,$nombre,$descripcion,$precio_creacion,$precio_venta,$medida,$categoria,$id);            //trae el metodo editar usuario de Model
                if ($data =="modificado") {
                    $msg = "modificado";
                }else{
